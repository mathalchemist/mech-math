...
void getp(int i,int j, TVector * p11, TVector * p12, TVector * p21, TVector * p22)
{
    double xx;
    double yy;
    double ii = double(i);
...
//    printf("SFB: %2.5lf\n", SOMEWHERE_FAR_BEYOND);
	...
	if (pdist < 0.02) { vis = true; break; }
	if (fdist < 0.015 && pdist > .2) { vis = false; break; }
    }
    return vis;
}
...
int KeyPressFunction (int nKeySym)
{
  ...
  case XK_Return:coeff_x = 1.0;coeff_y = 1.0;break;

  case XK_F1:coeff_x *= 1.3; break;
  case XK_F2:coeff_x /= 1.3; break;

  case XK_F3:coeff_y *= 1.3; break;
  case XK_F4:coeff_y /= 1.3; break;

  case XK_F5: N *=2; break;
  case XK_F6: if (N > 4) N /= 2; break;

  case XK_F7: ph += 0.05; break;
  case XK_F8: ph -= 0.05; break;

  case XK_A:case XK_a: C += TVector(0,0,.1); break;
  case XK_Z:case XK_z: C -= TVector(0,0,.1); break;

  case XK_D:case XK_d: O += TVector(0,0,.1); break;
  case XK_C:case XK_c: O -= TVector(0,0,.1); break;

  case XK_F:case XK_f: SOMEWHERE_FAR_BEYOND += 1.0; break;
  case XK_V:case XK_v: SOMEWHERE_FAR_BEYOND -= 1.0; break;

  case XK_S:case XK_s: S *= 1.1; break;
  case XK_X:case XK_x: S /= 1.1; break;
...
void GetMem(int n, double * x, double * fv, double * xi, double * aa, double * b, double * v)
{
    x = new double[n];fv = new double[n];xi = new double[n+1];
    v = new double[n+1];aa = new double[(n+1)*(n+1)];bb = new double[n+1];
    c1 = new double[n*n];c2 = new double[n*n];c3 = new double[n*n];
}

void FreeMem(double * x, double * fv, double * xi, double * aa, double * b)
{
    delete x;delete fv;delete xi;delete aa;delete bb;delete v;
...
void GetC_i(int i)
{
    ...
    C1(i) = V(i);
    C2(i) = (F(i) - V(i)) / (X(i)-XI(i)) - (X(i)-XI(i))/(XI(i+1)-XI(i))*fr;
...
void DrawWindowContent()
{
    ...
    WDrawLine(w, h/2, w-5,h/2+5);
    WSetColor(RED);
    WDrawString ("Q=quit, F1..F4 -- scale, F5/F6 -- node count, F7/F8 -- add d(x)", 10, 20);
