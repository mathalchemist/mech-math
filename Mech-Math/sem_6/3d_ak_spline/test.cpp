...
double phi = 0; double psi = 0;//3.14/6.0; 
...
{
    if (method == 0) return Para(i,t); else return Pars(i,t);
}
...
void DrawWindowContent()
{
    ...
    zr.RotateZ(phi); zr.RotateX(psi);
    zl.RotateZ(phi); zl.RotateX(psi);
    yr.RotateZ(phi); yr.RotateX(psi);
    yl.RotateZ(phi); yl.RotateX(psi);
    xr.RotateZ(phi); xr.RotateX(psi);
    xl.RotateZ(phi); xl.RotateX(psi);
    
    ZZ.RotateZ(phi); ZZ.RotateX(psi);
    XX.RotateZ(phi); XX.RotateX(psi);
    YY.RotateZ(phi); YY.RotateX(psi);
    
    WSetColor(RED);
    // arrows
    WDrawLine(MX(ZZ.Coords[0]), MY(ZZ.Coords[2]), MX(zr.Coords[0]),MY(zr.Coords[2]));  
    WDrawLine(MX(ZZ.Coords[0]), MY(ZZ.Coords[2]), MX(zl.Coords[0]),MY(zl.Coords[2]));  
    WDrawLine(MX(YY.Coords[0]), MY(YY.Coords[2]), MX(yr.Coords[0]),MY(yr.Coords[2]));  
    WDrawLine(MX(YY.Coords[0]), MY(YY.Coords[2]), MX(yl.Coords[0]),MY(yl.Coords[2]));  
    WDrawLine(MX(XX.Coords[0]), MY(XX.Coords[2]), MX(xr.Coords[0]),MY(xr.Coords[2]));  
    WDrawLine(MX(XX.Coords[0]), MY(XX.Coords[2]), MX(xl.Coords[0]),MY(xl.Coords[2]));  
    // axes
	...
	if (drawint==0)
	{
	    if (method==0) GetWeights(n); else Interpolatesp(n);
	}
	...
	for (int j = 1; j < N; j++) // X
	{
	    ...
	    norm21.Coords[1] < lim && norm22.Coords[1] < lim) v = false;
	    if (vvv == 1) v = true;
...
//		int dwc = (ic << (8+8)) + (ic << 8) + ic;
		...
		WSetColor(dwc);
	        ...
	        if (drawint==0)
		{
		    if (method==0)WSetColor(GREEN);else WSetColor(YELLOW);
...
int KeyPressFunction (int nKeySym)
{
  switch (nKeySym)
  {
  case XK_Q:case XK_q:case XK_Escape:return KEY_PRESS_QUIT;
  
  case XK_Return: method = (method+1)%2;break;
  
  case XK_BackSpace: vvv = (vvv+1)%2;break;
  
  case XK_F: case XK_f: drawint=(drawint+1)%2; break;

  case XK_F1:coeff_x *= 1.2; break;
  case XK_F2:coeff_x /= 1.2; break;

  case XK_F3:coeff_y *= 1.2; break;
  case XK_F4:coeff_y /= 1.2; break;

  case XK_a: N *=2; InterpolateXY(N); break;
  case XK_z: if (N > 4) N /= 2; InterpolateXY(N); break;

  case XK_F7: if (phi <  3.14/3.0) phi += 0.025; break;
  case XK_F8: if (phi > -3.14/3.0) phi -= 0.025; break;

  case XK_F5:psi += 0.05; break;
  case XK_F6:psi -= 0.05; break;
