...
bool S_Reflect(double * a, int n, double * ai)
{
    ...
    for (int i = 0; i < n; i++)
	for (int j = 0; j < n; j++) ai[i*n+j] = (i == j);
	...
	for (int col = k+1; col < n; col++) // cycle through cols of A
	{
	    ...
	    double dp = 0.0;
	    for (int i = k; i < n; i++) dp += x[i]*A(i,col);
	    for (int i = k; i < n; i++) A(i,col) -= 2.0*x[i]*dp;
	}
	A(k,k) = Na;
	...
	{
	    double dp = 0.0;
	    for (int i = k; i < n; i++) dp += x[i]*ai[i*n+col];
	    for (int i = k; i < n; i++) ai[i*n+col] -= 2.0*x[i]*dp;
	}
    }
    delete x;
