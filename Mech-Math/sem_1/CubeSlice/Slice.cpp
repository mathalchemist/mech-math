...
public:
   TVector P[2];
   TSegment(void) {}
   TSegment(TVector P1, TVector P2) { P[0] = P1; P[1] = P2; }
};
...
void KeyboardHandler(TKeyInfo KInfo)
{
	...
	case 'z': RotateFaces(DirX, R_ANG); break;
        case 'x': RotateFaces(DirY, R_ANG); break;
        case 'c': RotateFaces(DirZ, R_ANG); break;
	case 'a': RotateFaces(DirX, -R_ANG); break;
        case 's': RotateFaces(DirY, -R_ANG); break;
        case 'd': RotateFaces(DirZ, -R_ANG); break;
	default: break;
    }
    ProjectFaces();
}
...
bool CalcSlice(void)
{
	...
	double D = V | N;
	if (D == 0) continue; // skip lines parallel to plane
	double t = (N | (P - A)) / D;
	if (Eq(A) * Eq(B) > 0) continue; // skip points that doesn't belong to segment.
	if (NumPoints > 5) return false; // Oh, shit, SEGMENTATION FAULT EXPECTED!!!
	SP[NumPoints] = A + V * t;
	NumPoints++;	
    }
...
void RotateFaces(TVector D, double Angle)
{
   ...
   {
      for (int J = 0; J < 4; J++) Faces[I].P[J].Rotate(&D, Angle);
      Faces[I].Normal.Rotate(&D, Angle);
   }
   for (int I = 0; I < NumPoints; I++) SP[I].Rotate(&D, Angle);
}
...
bool PointInSlice(int x, int y, TSP * P)
{
	...
	for (int J = 1; J < NumPoints; J++)
		...
		if (I != J && I!= K && J != K)
			...
			LineEq(P[K], P[I], x, y) * LineEq(P[K], P[I], P[J]) >= 0) return true;
    return false;	
}
...
void ProjectFaces(void)
{
    ...
    for (int I = 0; I < NumPoints; I++)
    {
        SSP[I].x = int(R_SCALE * SP[I].Coords[0] * Cz / ((SP[I].Coords[2] + 3) - Cz));
        SSP[I].y = int(R_SCALE * SP[I].Coords[1] * Cz / ((SP[I].Coords[2] + 3) - Cz));
    }
    // 1.2) project cube wireframe points
    for (int I = 0; I < MAX_FACES; I++)
	for (int J = 0; J < 4; J++)
    	{	
    	    CSP[I * 4 + J].x = int(R_SCALE * Faces[I].P[J].Coords[0] * Cz / ((Faces[I].P[J].Coords[2] + 3) - Cz));
    	    CSP[I * 4 + J].y = int(R_SCALE * Faces[I].P[J].Coords[1] * Cz / ((Faces[I].P[J].Coords[2] + 3) - Cz));
    	}
	...
	if (Faces[I].Normal.Coords[2] >= 0)
    	    for (int J = 0; J < 4; J++) XWin->DrawLine(R_X/2 + CSP[I * 4 + J].x, R_Y/2 - CSP[I * 4 + J].y, R_X/2 + CSP[I * 4 + (J + 1) % 4].x, R_Y/2 - CSP[I * 4 + (J + 1) % 4].y, RGB(84, 84, 84));
    ...
    {
	if (SSP[I].x < Left) Left = SSP[I].x;
	if (SSP[I].x > Right) Right = SSP[I].x;
	if (SSP[I].y < Bottom) Bottom = SSP[I].y;
	if (SSP[I].y > Top) Top = SSP[I].y;
    }
    // 2.2.2) check all the points in rectangle and paint matching points.
    for (int x = Left; x <= Right; x++)
	for (int y = Bottom; y <= Top; y++)
	    if (PointInSlice(x, y, SSP)) XWin->DrawPoint(R_X/2 + x, R_Y/2 - y, RGB(0, 0, 255));
