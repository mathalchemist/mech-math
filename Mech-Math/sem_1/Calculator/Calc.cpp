...
void TNum::Write(char * FileName)
{
   ...
   for (int I = GetLength() - 1; I >= 0; I--) fprintf(F, "%d", Data[I]);
   if (!GetLength()) fprintf(F, "%d", 0);
   fclose(F);
}

void TNum::Display(void)
{
   for (int I = GetLength() - 1; I >= 0; I--) printf("%d", Data[I]);
   if (!GetLength()) printf("%d", 0);
}

int TNum::GetLength(void)
{
   int Length;
   for (int I = MAX_LENGTH - 1; I >= 0; I--) if (Data[I]) return I + 1;
   return 1;
}
...
{
   int L = N.GetLength();
   for (int I = 0; I < L; I++) Data[I] = N.Data[I];
   return *this;
}

bool TNum::operator == (TNum& N)
{
   for (int I = 0; I < MAX_LENGTH; I++) if (Data[I] != N.Data[I]) return false;
   return true;
}
...
TNum& TNum::operator -= (TNum& N)
{
   if (N != 0) for (TNum I(0); I != N; I++) (*this)--;
   return *this;
}
   ...
   if (N != 0)
      for (int I = 0; I < L; I++) // cycle through multiplier digits
      {
         ...
         for (int J = 0; J < MAX_LENGTH; J++)
         {
            ...
            if (I + J < MAX_LENGTH) A.Data[I + J] = P;
         }
         Result += A;
      }
   *this = Result;
