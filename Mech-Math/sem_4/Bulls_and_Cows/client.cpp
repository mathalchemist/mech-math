...
int main(int argc, char *argv[])
{
    ...
    if (signal(SIGPIPE, &SigHandler) == SIG_ERR) { printf("Error: cannot set signal handler!\n"); return 0; }

    // Create a socket
    int s0 = socket(AF_INET, SOCK_STREAM, 0);
    if (s0 < 0) { printf("Error: cannot create a socket.\n"); return 0; }

    // Fill in the address of server
    sockaddr_in SrvAddr;
    memset(&SrvAddr, 0, sizeof(SrvAddr));
    
    char * HostName = "localhost"; if (argc >= 2) HostName = argv[1];

    // Resolve the server address (convert from symbolic name to IP number)
    hostent * Host = gethostbyname(HostName);
    if (!Host) { printf("Error: cannot get host address.\n"); close(s0); return 0; }
    
    SrvAddr.sin_family = AF_INET;
    int Port = 1234; if (argc >= 3) Port = atoi(argv[2]);
    ...
    if (connect(s0, (struct sockaddr*) &SrvAddr, sizeof(SrvAddr)) < 0) { printf("Error: cannot connect to remote host.\n"); close(0); return 0; }
    
    char IP[4]; ExtractIP(Host->h_addr_list[0], IP);
    ...
    do { printf("Enter number:"); scanf("%d", &MyNumber); } while (MyNumber < 1023 || MyNumber > 9876);
    

    // program cycle
    int SendBuffer[MAX]; int RecvBuffer[MAX];
    int TN = 0;
    while (true)
    {
	...
	if (!WaitMessage(s0, RecvBuffer)) break;
	...
	if (HisNumber == MyGuess) printf("CONGRATULATIONS: You win wins at the %d-th turn!.", TN);
    }
    close(s0);
    return 0;
}
...
bool GetAnswer(int N, int GN, int * B, int * C)
{
    ...
    int b = 0; int c = 0;
    if (gd0 == d0) b++; if (gd1 == d1) b++; if (gd2 == d2) b++; if (gd3 == d3) b++;
