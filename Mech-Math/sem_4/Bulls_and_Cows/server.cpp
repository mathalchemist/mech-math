...
void ExtractIP(sockaddr_in A, char * IP)
{
    ...
    for (int i = 0; i < 4; i++) { IP[i] = B % 0x100; B /= 0x100; }
}
...
int main(int argc, char * argv[])
{
    ...
    if (bind(s0, (sockaddr*) &MyAddr, sizeof(MyAddr)) < 0) { printf("Error: cannot bind a socket.\n"); close(s0); return 0; }
    printf("Waiting for the client...\n");
    linger linger_opt = { 1, 0 }; 
    setsockopt(s0, SOL_SOCKET, SO_LINGER, &linger_opt, sizeof(linger_opt));
    if (listen(s0, 1) < 0) { printf("Error: cannot listen on the socket.\n"); close(s0); return 0; }

    sockaddr_in CliAddr;
    socklen_t AddrLen;
    int s1 = accept(s0, (struct sockaddr*) &CliAddr, &AddrLen);
    if (s1 < 0) { printf("Error: cannot accept connection.\n"); return 0; }

    char IP[4]; ExtractIP(CliAddr, IP);
    printf("Accepted connection from IP %d.%d.%d.%d on port %d\n", IP[0], IP[1], IP[2], IP[3], ntohs(CliAddr.sin_port));
