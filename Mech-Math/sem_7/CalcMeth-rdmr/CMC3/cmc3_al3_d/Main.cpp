...
int main()
{
  ...
  for (int i = 0; i < 20000; i++)
  {
    ...
    x += ddx*c;
    y += ddy*c;
    if (l2(x,y)>200) { printf("unstable point! \n"); return -1; }
  }
  ...
  for (int i =  0; i < 12000; i++)
  {
    ...
    x += ddx*c;
    y += ddy*c;
    if (l2(x,y)>100) { printf("unstable point! \n"); return -2; }
    if (d(x_0,y_0,x,y)<1e-2 && i > 100)
    {
    returned = true;
      printf("Time: $%1.5lf$. Length: $%1.5lf$, Point on the cycle: $(%1.5lf, %1.5lf)$\n", time, length,x,y);
      break;
    }
  }
  if (!returned) printf("Trajectory has not returned.\n");
