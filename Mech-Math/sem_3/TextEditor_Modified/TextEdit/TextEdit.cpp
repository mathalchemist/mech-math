...
TextEdit::TextEdit():  GWindow(), text(), cursorX(0), cursorY(0), windowX(0), windowY(0), windowWidth(80),    // Window size in characters (dx, dy)
    windowHeight(24), lastChar(0), textFont(0), fontStruct(), fontName(0),        // Font name (in X11 form)
    dx(0), ascent(0), descent(0), leading(0), dy(0), leftMargin(4), topMargin(4), statusLineMargin(4),
    rightMargin(4), bottomMargin(4), fileName("noname.txt"), fileNameSet(false),
    endOfText("[* End of text *]"), textChanged(false), textSaved(false), inputDisabled(false),
    focusIn(true), bgColor(0), fgColor(0), bgStatusLineColor(0), fgStatusLineColor(0) {}
...
//    if (fontString != 0) { if ((textFont = loadFont(fontString)) != 0) success = true; }
  ...
  //  if (!success) { if ((textFont = loadFont(DEFAULT_TEXT_FONT)) != 0) success = true; }
    ...
    if (fStruct == 0) { perror("Could not load a font"); exit(1); }
    fontStruct = *fStruct;
}

bool TextEdit::loadFile(const char* filePath) { setFileName(filePath); fileNameSet = true; return text.load(filePath); }

void TextEdit::redrawStatusLine() { redrawRectangle(I2Rectangle(0, 0, m_IWinRect.width(), dy + statusLineMargin)); }
...
void TextEdit::setFileName(const char* filePath) { fileName = filePath; fileNameSet = true; } 
...
void TextEdit::onDeleteWord()
{
    ...
    if (cursorX >= l.length()) return;
    
    if (l[cursorX] != ' ') // WORD
    {
      while (cursorX > 0 && l[cursorX]!= ' ') cursorX--;
      if (l[cursorX] == ' ') cursorX++;
      while (cursorX < l.length())
        if (l[cursorX] != ' ') l.removeAt(cursorX); else break;
    }
    else // Spaces
    {
      while (cursorX > 0 && l[cursorX]== ' ') cursorX--;
      if (l[cursorX] != ' ') cursorX+=2;
      while (cursorX < l.length())
        if (l[cursorX] == ' ') l.removeAt(cursorX); else break;
    }
