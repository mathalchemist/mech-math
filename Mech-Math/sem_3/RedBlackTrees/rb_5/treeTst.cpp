...
public:
    ...
    virtual int compareTo(const RBTreeNodeValue& v) const { return (number - ((const Integer&) v).number); }
    virtual ~Integer() {}
};
...
void GetMaxPath(RBTreeNode *node)
{
  ...
  int ld = GetDepth(l);
  int rd = GetDepth(r);
  if (node->red) printf("[red "); else printf("[black ");
  Integer* v = (Integer*) node->value;
  printf("%d] ", v->number);
  if (ld >= rd) GetMaxPath(l); else GetMaxPath(r);
}
...
void GetMaxRedPath(RBTreeNode *node)
{
  ...
  Integer* v = (Integer*) node->value;
  printf("%d] ", v->number);
  if (ld >= rd) GetMaxRedPath(l); else GetMaxRedPath(r);
}
