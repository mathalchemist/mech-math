...
void ExposeHandler(void) { ProjectFaces(); }
...
void ProjectFaces() {
  ...
  for (int I = 0; I < MAX_FACES; I++)
    ...
    {
      for (int J = 0; J < 4; J++) {
        SP[J].x = int(R_SCALE * Faces[I].P[J].Coords[0] * Cz /
                      ((Faces[I].P[J].Coords[2] + 3) - Cz));
        SP[J].y = int(R_SCALE * Faces[I].P[J].Coords[1] * Cz /
                      ((Faces[I].P[J].Coords[2] + 3) - Cz));
      }
      for (int J = 0; J < 4; J++)
        XWin->DrawLine(R_X / 2 + SP[J].x, R_Y / 2 - SP[J].y,
                       R_X / 2 + SP[(J + 1) % 4].x, R_Y / 2 - SP[(J + 1) % 4].y,
                       RGB(255, 255, 255));
        }
    // axis
