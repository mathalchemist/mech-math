...
//заполняет массиы матрик block_n x block_n + rest_n x rest_n единичными
//матрицами
...
static inline void MUL1(double *A, double *B, double *C, double *T1, double *T2,
                        int block_n) {
  int i, j, k;
  double f;
...
//вычисляет A_t x B x C, передаются матрица A нетранспонипованная
static inline void MUL2(double *A, double *B, double *C, double *T1, double *T2,
                        int block_n, int rest_n) {
  int i, j, k;
  double f;
  double *t1 = T1, *c = C, *a = A;
  // B x C результат в T1
...
//вычисляет A_t x B x C, передаются матрица A нетранспонипованная
static inline void MUL3(double *A, double *B, double *C, double *T1, double *T2,
                        int block_n, int rest_n) {
  int i, j, k;
  double f;
  double *t1 = T1, *c = C, *a = A;
...
//|an+11     .. ann+1 an+1n+1|
// A_ij - квадратные block_n x block_n, а a_ij - прямоугольные и квадратные
// соотв
// block_n * .. и .. x ..

int Solve_Chkolecski(
    double *A, //матрица системы, n x n
    double *b, //правая часть системы, n
    double *x, //решение, n
    double *C1,
    double
        *C2,    //временные матрицы для промежуточных операций, block_n x block_n
    double *C3, //массив матриц, обратных к диагональным матрицам, n x block_n +
                //(n % block_n)^2
    double *C4,
    double *C5,       //временные матрицы для перемножения, block_n x block_n
  ...
  int N = block_count, block_per_thread, first_block, last_block,
      additional_block = 0;
  ...
  for (i = 0; i < block_count; i++, N--) {
    ...
    //если осталось active_threads-1 блоков, то распределяем их поровну между
    //1..active_threads-1,
    //при этом если есть неквадратный блок, то он обрабатывается задачей 0.
    //иначе оставшиеся блоки обрабатываются задачами 1..N%active_threads, а
    //оставшийся неквадаратный
    ...
    if (thread_num < active_threads) {
      ...
      if (N % active_threads == active_threads - 1) {
        if (thread_num != 0)
          additional_block =
              i + block_per_thread * active_threads + (thread_num - 1);
        else
    ...
    if (thread_num == 0) {
      ...
      if (!Back_Jordan_C(C1, C3 + block_n_x_block_n * i, block_n))
        return 0;
    }
  ...
  if (rest_n != 0) {
    ...
    if (!Back_Jordan_C(C1, C3 + block_n_x_block_n * block_count, rest_n))
      return 0;
  }
