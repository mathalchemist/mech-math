...
void QRReflect(real * a, int n, int e, real * q)
{
	...
	x0 -= Na;
	Nx = sqrt(x0*x0+s);
	if (Nx < eps) { printf("QR_Reflect(): Bad norm Nx!\n"); exit(1); }
	Nx = 1.0 / Nx;
...
void S_Reflect(real acc, real * a, int n, real * q, real * ev)
{
    ...
    {
	printf("3-Diag: k = %d\n", k);
	s = 0.0; for (int i=k+2;i<n;i++) { x[i] = A(i,k); s+=x[i]*x[i]; }
	t = A(k+1,k);
	Na = sqrt(t*t+s);
	t -= Na;
	x[k+1] = t;
	Nx = sqrt(s + t*t);
	if (Nx < eps) { printf("S_Reflect(): Bad matrix!\n"); exit(1); }
	Nx = 1.0 / Nx; for (int i=k+1;i<n;i++) x[i] *= Nx;
	
	for (int col = k + 1; col < n; col++) // cycle through cols of A
	{
	    real dp = 0.0; for (int i = k+1; i < n; i++) dp += x[i]*A(i,col);
	    dp *= 2.0; for (int i = k+1; i < n; i++) A(i,col) -= x[i]*dp;
	}
	A(k+1,k) = Na; for (int i = k+2; i < n; i++) A(i,k) = 0.0; // cleanup
	for (int row = k + 1; row < n; row++) // cycle through cols of A
	{
	    real dp = 0.0; for (int i = k+1; i < n; i++) dp += x[i]*A(row,i);
	    dp *= 2.0; for (int i = k+1; i < n; i++) A(row,i) -= x[i]*dp;
	}
	A(k,k+1) = Na; for (int j = k+2; j < n; j++) A(k,j) = 0.0; // cleanup
    }
    delete x;
	...
	while (fabs(A(k-1,k-2)) > acc * Row_norm)
	{
	    ...
	    s = A(k-1,k-1); for (int i = 0; i < k; i++) A(i,i) -= s;
#endif
    	    QRReflect(a, n, k, q);
//	    printf("Matrix R, k = %d:\n", k); PrintMatrix(a, 0, n);
	    //printf("Matrix Q, k = %d:\n", k); PrintMatrix(q, 0, n);
...
//	    printf("Matrix A:\n"); PrintMatrix(a, 0, n);
    ...
    if ( n > 1)
    {
	real t = a[0]+a[n+1]; real d = a[0]*a[n+1]-a[1]*a[n];
	d = t*t-4*d;
	if (d < 0) printf("suxx: inconsistent equation!!!\n");
	else { d= sqrt(d); a[0] = (t+d)/2.0; a[n+1] = (t-d)/2.0; }	
    }
...
void PrintMatrix(real * arr, int k, int n)
{
    for (int i = k; i < n; i++)
    {
	for (int j = k; j < n; j++) printf("%1.8Lf ", arr[i*n+j]);
	printf("\n");
    }
}
