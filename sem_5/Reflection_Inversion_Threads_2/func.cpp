...
bool S_Reflect(int ThreadCount, double *a, int n, double *ai) {
  ...
  if (!iRest)
    iFullThreads = ThreadCount;
  else
    iFullThreads = ThreadCount - 1;
  int iColsPerThread = 0;
  if (iFullThreads)
    iColsPerThread = n / iFullThreads; // cols per thread
  ...
  for (int k = 0; k < n - 1; k++) // steps. let's begin...
  {
    ...
    if (!Rest)
      ...
      FullThreads = ThreadCount - 1;
    int ColsPerThread = 0;
    if (FullThreads)
      ColsPerThread = TotalCols / FullThreads; // cols per thread
