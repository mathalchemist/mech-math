...
void fill_cl(double *c, int k, int l) {
  for (int j = 1; j < N; j++)
    for (int i = 1; i < N; i++)
      C(i, j) = 0;
  for (int i = 1; i < N; i++)
    C(i, i) = 4 - 2.0 * cos(double(2 * l - 1) * 3.1415926 / double(1 << k));
...
int main() {
  ...
  for (int j = 1; j < N - 1; j++) {
    for (int i = 1; i < N - 1; i++) {
      fprintf(FFF, "draw dpr((%1.3lf*u, %1.3lf*u, %1.3lf*u))--dpr((%1.3lf*u, "
                   "%1.3lf*u, %1.3lf*u));\n",
              double(i) * h, double(j) * h, Y(j, i), double(i + 1) * h,
              double(j) * h, Y(j, i + 1));
      fprintf(FFF, "draw dpr((%1.3lf*u, %1.3lf*u, %1.3lf*u))--dpr((%1.3lf*u, "
                   "%1.3lf*u, %1.3lf*u));\n",
              double(i) * h, double(j) * h, Y(j, i), double(i) * h,
              double(j + 1) * h, Y(j + 1, i));
    }
    fprintf(FFF, "\n");
  }
  ...
  for (int j = 1; j < N - 1; j++) {
    for (int i = 1; i < N - 1; i++) {
      fprintf(FFF, "draw dpr((%1.3lf*u, %1.3lf*u, %1.3lf*u))--dpr((%1.3lf*u, "
                   "%1.3lf*u, %1.3lf*u)) wc red;\n",
              double(i) * h, double(j) * h, g(j, i), double(i + 1) * h,
              double(j) * h, g(j, i + 1));
      fprintf(FFF, "draw dpr((%1.3lf*u, %1.3lf*u, %1.3lf*u))--dpr((%1.3lf*u, "
                   "%1.3lf*u, %1.3lf*u)) wc red;\n",
              double(i) * h, double(j) * h, g(j, i), double(i) * h,
              double(j + 1) * h, g(j + 1, i));
    }
    fprintf(FFF, "\n");
  }
