...
static int variant = 0, N = 300, n_appr = 50;

static double a = -3.14, b = 3.14, y_min, y_max, *x_val, *f_val, *coeffs1,
              *coeffs2;

double fun(double z) { return z + sin(z) + cos(z) * cos(z); }
...
void plot_fun(double (*fun)(double z), double a, double b, int N) {
  ...
  double *x, *f, max_val, min_val, u_x, u_y, x_start, y_start, x_end, y_end;
...
void plot_2fun(double (*fun1)(double z), double (*fun2)(double z), double a,
               double b, int N) {
  int j, x_c, y_c, width = nWWidth, height = nWHeight;
  double *x, *f1, *f2, max_val, min_val, u_x, u_y, x_start, x_end, y_start1,
      y_end1, y_start2, y_end2;
...
void draw_axes_and_labels() {
  ...
  for (j = 1; j <= 5; j++) {
    ...
    WDrawString(s, width * 71. / 140,
                height * (19. / 20. - 9. / 10. * (5 * j + 1) / 30.));
    free(s);
  }
...
static void DrawWindowContent() {
  int width = nWWidth, height = nWHeight;
