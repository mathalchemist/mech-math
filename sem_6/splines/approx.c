...
void poly_3val_der(FPT x1, FPT x2, FPT x3, FPT f1, FPT f2, FPT f3, FPT d,
                   FPT *coeffs) {
  /*
  ...
  */
  ...
  coeffs[3] = (((f3 - f2) / (x3 - x2) - (f2 - f1) / (x2 - x1)) / (x3 - x1) -
               coeffs[2]) /
              (x3 - x1);
...
void Bessel_approx(int n, FPT *x, FPT *f, FPT *coeffs) {
  ...
  */
  ...
  d2 = 0;
  poly_3val_der(x[2], x[0], x[1], f[2], f[0], f[1], d1, coeffs);
  for (j = 0; j <= 3; j++)
    coeffs[j + 4] = coeffs[j];
  ...
  poly_3val_der(x[n - 3], x[n - 2], x[n - 1], f[n - 3], f[n - 2], f[n - 1], d2,
                coeffs + 4 * (n - 3));
...
void spline_approx(int n, FPT *x, FPT *f, FPT *coeffs) {
  ...
  */
  ...
  d[0] = x[2] - x[1];
  u[0] = x[2] - x[0];
  r[0] = ((f[1] - f[0]) / (x[1] - x[0]) * (x[2] - x[1]) *
              (2 * x[2] + x[1] - 3 * x[0]) +
          (f[2] - f[1]) / (x[2] - x[1]) * (x[1] - x[0]) * (x[1] - x[0])) /
         (x[2] - x[0]);
  ...
  d[n - 1] = x[n - 2] - x[n - 3];
  r[n - 1] =
      ((f[n - 2] - f[n - 3]) / (x[n - 2] - x[n - 3]) * (x[n - 1] - x[n - 2]) *
           (x[n - 1] - x[n - 2]) +
       (f[n - 1] - f[n - 2]) / (x[n - 1] - x[n - 2]) * (x[n - 2] - x[n - 3]) *
           (3 * x[n - 1] - x[n - 2] - 2 * x[n - 3])) /
      (x[n - 1] - x[n - 3]);
  ...
  for (j = 0; j <= n - 2; j++) {
    ...
    coeffs[4 * j + 3] =
        (d[j] + d[j + 1] - 2 * div_dif) / (x[j + 1] - x[j]) / (x[j + 1] - x[j]);
  }
...
FPT eval_cubic_approx(FPT x0, int n, FPT *x, FPT *coeffs) {
  ...
  */
  ...
  if (f)
    return ((coeffs[4 * j + 3] * (x0 - x[j]) + coeffs[4 * j + 2]) *
                (x0 - x[j]) +
            coeffs[4 * j + 1]) *
               (x0 - x[j]) +
           coeffs[4 * j];
  else {
    printf("Error in eval_cubic_approx : x value, %5.2f, out of interval "
           "(%5.2f;%5.2f)\n",
           x0, x[0], x[n - 1]);
    return 0;
  }
