...
const int WDef = 800; const int HDef = 600;
const int WMin = 50;  const int HMin = 50;
const int WBorder = 5;

const char * WTitle = "Function interpolation";
const char * WITitle = WTitle;
const char * PClass = "FIClass";
...
int AllocColors (Display * prDisplay, Colormap colormap, COLORS * colors)
{
  ...
  if (!XANC("black")) return 1; colors->black = rColor.pixel;
  if (!XANC("blue")) return 2; colors->blue = rColor.pixel;
  if (!XANC("green")) return 3; colors->green = rColor.pixel;
  if (!XANC("cyan")) return 4; colors->cyan = rColor.pixel;
  if (!XANC("red")) return 5; colors->red = rColor.pixel;
  if (!XANC("magenta")) return 6; colors->magenta = rColor.pixel;
  if (!XANC("brown")) return 7; colors->brown = rColor.pixel;
  if (!XANC("light gray")) return 8; colors->light_gray = rColor.pixel;
  if (!XANC("dark gray")) return 9; colors->dark_gray = rColor.pixel;
  if (!XANC("light blue")) return 10; colors->light_blue = rColor.pixel;
  if (!XANC("light green")) return 11; colors->light_green = rColor.pixel;
  if (!XANC("light cyan")) return 12; colors->light_cyan = rColor.pixel;
  if (!XANC("yellow")) return 13; colors->yellow = rColor.pixel;
  if (!XANC("white")) return 14; colors->white = rColor.pixel;
  return 0;
}
...
{ for (int i = 0; i <= 16; i++) XFreePixmap (prDisplay, prPixmap[i]); }
...
COLORS rColors;
int nWWidth = WDef; int nWHeight = HDef;

// handlers

bool HandleExpose(void (*DrawWindowContent)(), XEvent Event, XWindowAttributes Attr, int Depth)
{
    if (Event.xexpose.count != 0) return 0;
    if (XGetWindowAttributes(prDisplay, nWnd, &Attr))
    {
	if (nWWidth != Attr.width || nWHeight != Attr.height || Depth != Attr.depth)
	{
...
// true if quit pressed
bool HandleKeyPress(int (*KeyPressFunction)(int), XEvent Event/*, KeySym nKeySym*/)
{
    KeySym nKeySym;
...
//    KeySym nKeySym;
    ...
    nWnd = XCreateSimpleWindow(prDisplay,
	    RootWindow(prDisplay,nScreenNum),XPos,YPos,
	    WDef,HDef,
	    WBorder,
	    BlackPixel(prDisplay,nScreenNum),
	    WhitePixel(prDisplay,nScreenNum));
    if (SetWindowManagerHints(prDisplay,PClass,
	nWnd,WMin,HMin,WTitle,WITitle,0)
    ) return 2;
	...
	case Expose:
	    ...
	    break;
	case KeyPress:
	    if (HandleKeyPress(KeyPressFunction, Event/*, nKeySym*/)) return 0;
	    break;
	}
...
void WDrawString (const char *string, int x, int y) { XDrawString (prDisplay, draw, prGC, x, y, string, strlen (string)); }
void WDrawPoint (int x, int y) { XDrawPoint (prDisplay, draw, prGC, x, y); }
void WDrawPoint (double x, double y) { XDrawPoint (prDisplay, draw, prGC, (int) x, (int) y); }

void WDrawLine (int x_start, int y_start, int x_end, int y_end) { XDrawLine (prDisplay, draw, prGC, x_start, y_start, x_end, y_end); }
void WDrawLine (double x_start, double y_start, double x_end, double y_end) { XDrawLine (prDisplay, draw, prGC, (int) x_start, (int) y_start, (int) x_end, (int) y_end); }
