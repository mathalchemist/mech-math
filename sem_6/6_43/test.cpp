...
/* double SX(int i) { return a + double(i/nWWidth)*(b-a); } double SY(int i) { return 1.0 - double(i)/double(nWWidth); } */
...
//    double sttp = 0.01;
//    for (double t = a+0.05; t <=b-0.05; t+= sttp) { M += sttp * (fabs(f(t) - Par(t))); }
//    printf("Diff = %1.10lf\n", M);
