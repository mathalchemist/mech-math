...
const struct TextEdit::CommandDsc TextEdit::editorCommands[] = {
    ...
    {XK_a, ControlMask, ControlMask, true, &TextEdit::onDeleteWord}, // Ctrl+a
    {XK_A, ControlMask, ControlMask, true, &TextEdit::onDeleteWord}, // Ctrl+A
...
TextEdit::TextEdit()
    : GWindow(), text(), cursorX(0), cursorY(0), windowX(0), windowY(0),
      windowWidth(80), // Window size in characters (dx, dy)
      windowHeight(24), lastChar(0), textFont(0), fontStruct(),
      fontName(0), // Font name (in X11 form)
      dx(0), ascent(0), descent(0), leading(0), dy(0), leftMargin(4),
      topMargin(4), statusLineMargin(4), rightMargin(4), bottomMargin(4),
      fileName("noname.txt"), fileNameSet(false),
      endOfText("[* End of text *]"), textChanged(false), textSaved(false),
      inputDisabled(false), focusIn(true), bgColor(0), fgColor(0),
      bgStatusLineColor(0), fgStatusLineColor(0) {}
...
void TextEdit::loadTextFont() {
  ...
  //    if (fontString != 0) { if ((textFont = loadFont(fontString)) != 0)
  //    success = true; }

  // Then, try to load the default fomt
  //  if (!success) { if ((textFont = loadFont(DEFAULT_TEXT_FONT)) != 0) success
  //  = true; }
...
void TextEdit::onHome() { cursorX = 0; }
...
void TextEdit::onDeleteWord() {
  TextLine &l = text.getLine(cursorY);
  if (cursorX >= l.length())
    return;

  if (l[cursorX] != ' ') // WORD
  {
    while (cursorX > 0 && l[cursorX] != ' ')
      cursorX--;
    if (l[cursorX] == ' ')
      cursorX++;
    while (cursorX < l.length())
      if (l[cursorX] != ' ')
        l.removeAt(cursorX);
      else
        break;
