...
protected:
  ...
  void onButtonPress(XEvent &event) {
    ...
    setForeground("white");
    sprintf(line, "Click in %s, m_Window=%d", m_WindowTitle, (int)m_Window);
    drawString(x, y, line);
  }
  ...
  void onKeyPress(XEvent &) {
    char line[256];
    setForeground("white");
    sprintf(line, "Key pressed in %s, m_Window=%d", m_WindowTitle,
            (int)m_Window);
...
int main() {
  ...
  w1.createWindow(I2Rectangle(100, 100, 400, 300),
                  R2Rectangle(-2., -1.5, 4., 3.), "Brown Window");
  MyWindow w2;
  ...
  w2.createWindow(&w1,            // parent window
                  1,              // border width
