...
public:
  MyWindow()
      : // Constructor
        lastClick(),
        clicked(false), mouseButton(0) {}

  double f(double x);  // Function y = f(x)
  double df(double x); // Function y = f(x)
...
// Function y = f(x)
//
double MyWindow::f(double x) { return 1. / (1. + 0.1 * x * x); }

double MyWindow::df(double x) { return (f(x + 0.01) - f(x)) / 0.01; }
