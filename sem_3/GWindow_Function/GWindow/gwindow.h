...
public:
  ListHeader() : next(0), prev(0) {}

  ListHeader(ListHeader *n, ListHeader *p) : next(n), prev(p) {}

  ListHeader(const ListHeader &h) : next(h.next), prev(h.prev) {}
...
public:
  ...
  GWindow(const I2Rectangle &frameRect, const R2Rectangle &coordRect,
          const char *title = 0);
  virtual ~GWindow();
  ...
  void createWindow(const I2Rectangle &frameRect, const char *title = 0,
                    GWindow *parentWindow = 0,
                    int borderWidth = DEFAULT_BORDER_WIDTH);
  void createWindow(const I2Rectangle &frameRect, const R2Rectangle &coordRect,
                    const char *title = 0, GWindow *parentWindow = 0,
                    int borderWidth = DEFAULT_BORDER_WIDTH);
...
public:
  ...
  void drawAxes(const char *axesColorName = 0, bool drawGrid = false,
...
private:
  int clip(const R2Point &p1, const R2Point &p2, R2Point &c1, R2Point &c2);
};
