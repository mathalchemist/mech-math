...
public:
  ...
  bool between(const R2Point &a, const R2Point &b) const {
    R2Vector v(b - a);
    R2Vector m(*this - a);
    return (fabs(v.normal() * m) <= R2GRAPH_EPSILON && // point on line(a, b)
...
public:
  ...
  R2Rectangle(double left, double bottom, double width, double height)
      : l(left), b(bottom), w(width), h(height) {}

  R2Rectangle(const R2Point &leftBottom, double width, double height)
      : l(leftBottom.x), b(leftBottom.y), w(width), h(height) {}
...
public:
  ...
  I2Rectangle(int left, int top, int width, int height)
      : l(left), t(top), w(width), h(height) {}

  I2Rectangle(const I2Point &leftTop, int width, int height)
      : l(leftTop.x), t(leftTop.y), w(width), h(height) {}
