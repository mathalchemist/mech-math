...
public:
  ...
  TVector operator*(double Value) const {
  ...
  double Angle(const TVector &V) const {
    ...
    if (!L)
      return 0;
    return acos(((*this) | V) / L);
  }
};

inline TVector operator*(double Value, const TVector &V) {
  return TVector(Value * V.X, Value * V.Y, Value * V.Z);
}
...
class TPolynom {
  ...
  int Degree;
  int Capacity;
  TPolynom &RecalcDegree() {
    while (Degree > 0 && !Coeffs[Degree])
      Degree--;
    return *this;
  }
...
public:
  ...
  // Empty Constructor
  TPolynom(int InitDegree)
      : Degree(InitDegree), Capacity(Degree + RESERVED_SPACE) {
    Coeffs = new double[Capacity + 1];
    memset(Coeffs, 0, (Capacity + 1) * sizeof(double));
  }
  // Init Constructor
  TPolynom(int InitDegree, const double *InitCoeffs)
      : Degree(InitDegree), Capacity(Degree + RESERVED_SPACE) {
    Coeffs = new double[Capacity + 1];
    memset(Coeffs, 0, (Capacity + 1) * sizeof(double));
    for (int I = 0; I <= Degree; I++)
      Coeffs[I] = InitCoeffs[I];
  }
  // Copy Constructor
  TPolynom(const TPolynom &P)
      : Degree(P.Degree), Capacity(Degree + RESERVED_SPACE) {
    Coeffs = new double[Capacity + 1];
    memset(Coeffs, 0, (Capacity + 1) * sizeof(double));
    for (int I = 0; I <= Degree; I++)
      Coeffs[I] = P.Coeffs[I];
  ...
  TPolynom &operator+=(const TPolynom &P) {
    ...
    for (int I = 0; I <= P.Degree; I++)
      Coeffs[I] += P.Coeffs[I]; // This code shouldn't gluck because
  ...
  TPolynom &operator/=(double Value) {
    if (!Value)
      throw Exception("TPolynom::operator /= (Value): Division by zero");
    for (int I = 0; I <= Degree; I++)
      Coeffs[I] *= Value;
    return *this;
  }
...
public:
  ...
  TMatrix(int Width, int Height)
      : Arr(0), W(Width), H(Height) // init with zeros
  {
    Arr = new double[W * H];
    for (int j = 0; j < H; j++)
      for (int i = 0; i < W; i++)
  ...
  TMatrix(int Width, int Height, double *arr)
      : Arr(0), W(Width), H(Height) // init with data
  {
    Arr = new double[W * H];
    for (int j = 0; j < H; j++)
      for (int i = 0; i < W; i++)
  ...
  TMatrix operator*(const TMatrix &M) {
    if (W != M.H)
      throw Exception("Cannot multiply matrices"); // width of M1 = height of M2
    TMatrix R(M.W, H);
    for (int j = 0; j < R.H; j++)
      for (int i = 0; i < R.W; i++)
        for (int k = 0; k < W; k++)
          R.MATR(i, j) += MATR(k, j) * M.MATR(i, k);
    return R;
  }
  TMatrix &operator*=(const TMatrix &M) {
    if (W != M.H)
