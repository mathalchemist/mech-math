...
R2Vector R2Polygon::GetMassCenter() const {
  R2Vector mc(0, 0);
  for (R2Polygon::iterator i = begin(); i != end(); i++)
    mc += R2Vector(i->x, i->y);
  mc *= (1 / double(size()));
  return mc;
}
...
void R2Convex::addPoint(const R2Point &t) {
  if (m_NumAng == 0) {
    ...
    R2Point x = (m_A + m_B) * 0.5;
    m_MassCenter = R2Vector(x.x, x.y);
  } else if (m_NumAng == 2) {
    if (!R2Point::on_line(m_A, m_B, t)) {
      m_Polygon = new R2Polygon(m_A, m_B, t);
      R2Point x = (m_A + m_B + t) * (1 / 3.0);
      m_MassCenter = R2Vector(x.x, x.y);
      m_NumAng++;
    } else {
      if (m_B.between(m_A, t))
        m_B = t;
      else if (m_A.between(t, m_B))
        m_A = t; /* else leave them "as is" */
      R2Point x = (m_A + m_B) * 0.5;
      m_MassCenter = R2Vector(x.x, x.y);
    }
  } else {
