...
void MyWindow::onExpose(XEvent &event) { // Draw a window
  ...
  switch (PointStatus) {
  case PS_UNKNOWN:
    drawString(p, "Press 3rd button to test point");
    break;
  case PS_INSIDE:
    drawString(p, "Point is inside");
    break;
  case PS_OUTSIDE:
    drawString(p, "Point is outside");
    break;
  }
  if (n > 0) {
    setForeground("blue");
...
void MyWindow::onButtonPress(XEvent &event) {
  ...
  case Button2:
    if (conv.IsInside(t))
      PointStatus = PS_INSIDE;
    else
      PointStatus = PS_OUTSIDE;
    break;
  case Button3:
    conv.initialize();
    PointStatus = PS_UNKNOWN;
    break;
  }
  redraw();
}
...
int main() {
  ...
  // This crashes the main benefit of automatic class construction &
  // desturction.
  ...
  MyWindow w;
  w.createWindow(I2Rectangle(                   // Window frame rectangle:
