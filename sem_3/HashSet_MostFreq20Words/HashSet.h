...
public:
  ...
  bool operator!=(const HashSetKey &k) const { return !operator==(k); }
};
...
public:
  ...
  public:
    ...
    HashSetValue *value;
    Pair() : key(0), value(0) {}
    Pair(const HashSetKey *k, HashSetValue *v) : key(k), value(v) {}
  };
...
private:
  ...
  public:
    Pair pair;
    L1ListElement() : L1ListHeader(), pair() {}
    L1ListElement(HashSetKey *k, HashSetValue *v)
        : L1ListHeader(), pair(k, v) {}
    ~L1ListElement() {
      delete pair.key;
      delete pair.value;
    }
...
public:
  HashSet()
      : hashTableSize(1021), // Prime number
        hashTable(new L1ListHeader[hashTableSize]), numElements(0) {}

  HashSet(int tableSize)
      : hashTableSize(tableSize), hashTable(new L1ListHeader[hashTableSize]),
        numElements(0) {}
  ...
  HashSetValue *operator[](const HashSetKey *k) const { return value(k); }
...
public:
  ...
  public:
    ...
    const Pair &operator*() const { return element->pair; }
    const Pair *operator->() const { return &(operator*()); }
    bool operator==(const iterator &i) const {
      return (set == i.set && hash == i.hash && element == i.element);
    }
  ...
  public:
    ...
    const_iterator(const HashSet *s, int h) : iterator((HashSet *)s, h) {}
