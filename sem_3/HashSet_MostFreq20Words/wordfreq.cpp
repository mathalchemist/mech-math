...
public:
    ...
    virtual bool operator==(const HashSetKey& s) const { return (strcmp(str, ((Word&) s).str) == 0); }
...
Word::Word(const char* s, int l): str(new char(l+1)), len(l), capacity(len+1)
{ memmove(str, s, len); str[len] = 0; }

Word::Word(const Word& w): str(0), len(w.len), capacity(w.capacity)
{ if (w.len > 0) { str = new char[w.capacity]; memmove(str, w.str, capacity); } }

void Word::initialize() { len = 0; if (capacity > 0) str[0] = 0; }
...
Word& Word::operator+=(int c)
{
    if (capacity <= len+1) {
        ...
        delete[] str; capacity += extent; str = new_str;
    }
    str[len] = (char) c; ++len; str[len] = 0; return *this;
}
...
int Word::hashValue() const
{
    ...
    int hash = 0;
    for (int i = 0; i < len; i++) { hash *= HASH_FACTOR; hash += (int) str[i]; }
...
int main(int argc, char *argv[]) {
    ...
    if (argc > 1)
  {
    if (*argv[1] == '-') { printHelp(); return 0; }
        input = fopen(argv[1], "r");
        if (input == 0) { perror("Cannot open an input file"); return 1; }
    } else input = stdin;
    ...
    while (i != e)
  {
    ...
    {   //  если новое слово встречается чаще, чем самое редкое из имеющихся.
      if (number > Freqs[MinIndex]) { Words[MinIndex] = *w; Freqs[MinIndex] = number; }
