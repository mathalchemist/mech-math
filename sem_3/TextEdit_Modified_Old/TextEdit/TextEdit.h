...
// SF.Modifications: UNDO action
enum LASTACTIONS
{
    LA_UNDEF,	// Action that CANNOT be UNDO'ed (cursor movement, etc.)
    LA_CHARTYPED, // Character was typed
    LA_CHARERASED, // Character was erased by BackSpace/Delete
    LA_LINEINSERTED, // Empty line was added
    LA_LINEDELETED, // Line was removed
    LA_LINEBREAKED // after onEnter() event
};
...
protected:
    ...
    // (C) SegmentationFault Software, 2003
    void UndoLineDelete();
    void UndoLineBreak();
    void onUndo(); 	// UNDO COMMAND
    ...
    void redrawTextRectangle(int x, int y, int w, int h, bool createGC = true);
...
private:
    ...
    unsigned long buttonColor1; unsigned long buttonColor2; unsigned long buttonColor3;

public:
    enum { BUTTON_YES, BUTTON_NO, BUTTON_CANCEL } buttonPressed;
