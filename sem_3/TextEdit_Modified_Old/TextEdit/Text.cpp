...
TextLine::TextLine(): L2ListHeader(), capacity(0), len(0), str(0) {}

TextLine::TextLine(const TextLine& line): L2ListHeader(), capacity(line.capacity), len(line.len), str(0)
{
    if (capacity > 0) { str = new char[capacity]; memmove(str, line.str, len+1); }
}


TextLine::TextLine(const char* line): L2ListHeader(), capacity(0), len(0), str(0)
{ setString(line); }
...
void TextLine::setString(const char* line) {
    if (line == 0)
    {
        delete[] str; str = 0;
        len = 0; capacity = 0;
    } else setString(line, strlen(line));
}
...
TextLine& TextLine::operator=(const TextLine& line) { return *this; }

void TextLine::append(const char* line) {
    int l;
    if (line == 0 || (l = strlen(line)) == 0) return;
...
void TextLine::ensureCapacity(int n) {
    if (n > capacity)
    {
        ...
        if (optimalExtent < MIN_EXTENT) optimalExtent = MIN_EXTENT;
        else if (optimalExtent > MAX_EXTENT) optimalExtent = MAX_EXTENT;
        capacity += optimalExtent;
        if (capacity < n) capacity = n;

        // Make capacity to be a multiple of EXT_ALIGNMENT
        if ((capacity % EXT_ALIGNMENT) != 0) capacity += (EXT_ALIGNMENT - capacity % EXT_ALIGNMENT);
...
TextLine& TextLine::operator+=(const char* line) { append(line); return *this; }

TextLine& TextLine::operator+=(int character) { append(character); return *this; }

TextLine TextLine::operator+(const char* line) const { TextLine t(*this); t.append(line); return t; }

TextLine TextLine::operator+(int character) const { TextLine t(*this); t.append(character); return t; }
...
void TextLine::insert(int pos, const char* line) {
    ...
    if (pos < 0) pos = 0;
    if (pos > len) pos = len;
    if (pos < len) memmove(str + pos + l, str + pos, len - pos);
...
void TextLine::removeAt(int pos) {
    ...
    if (pos < len-1) memmove(str + pos, str + pos + 1, (len-1) - pos);
    --len;
    str[len] = 0;
    truncate(len);
}
...
bool Text::load(const char *filePath) {
    ...
    while (buffLen > 0) {
	    ...
	    {
                if (prevChar == '\r') { line.append(prevChar); pos++; }
                if (c == '\t')
		{
                    // Convert a tabulation into spaces
                    int spacesToAdd = tabWidth - (pos % tabWidth);
                    while (spacesToAdd > 0) { line.append(' '); pos++; spacesToAdd--; }
                }
		else if (c != '\r') { line.append(c); pos++; }
            }
            prevChar = c;
        }
        buffLen = fread(buffer, 1, 512, f);
...
bool Text::save(const char *filePath) const
{
    ...
    for (int k = 0; k < size() && i != e; ++k, ++i) {
        ...
        int l = line.size();
        if (l > 0) if (fwrite(line.getString(), 1, l, f) <= 0) { ret = false; break; } // Write a line
...
TextLine& Text::getLine(int n) { setPointer(n); return (TextLine&) elementAfter(); }

const char* Text::getString(int n) const {
    if (n < 0 || n >= size()) return 0;
    int pos = getPointerPosition();
    const_iterator i;
    if (n < pos) { i = endBefore(); while (pos != n) {  --i; --pos; } }
    else { i = beginAfter(); while (pos != n) { ++i; ++pos; } }
