...
public:
    const char *reason;
    L2ListException(): reason("") {}
    L2ListException(const char *cause): reason(cause) {}
};
...
public:
    ...
    L2ListHeader(): next(0), prev(0) {}
    L2ListHeader(L2ListHeader* n, L2ListHeader* p): next(n), prev(p) { }
    void link(L2ListHeader* h) { next = h; h->prev = this; }
...
public:
    L2List():
        ...
        pointer(&headList), numElements(0), pointerPos(0) { }

    void removeAll() { moveToBeg(); while (!inEnd()) removeAfter(); }

    ~L2List() { removeAll(); }

    void moveToBeg() { pointer = headList.next; pointerPos = 0; }
    void moveToEnd() { pointer = &headList; pointerPos = size(); }
    bool inBeg() { return (pointer->prev == &headList); }
    bool inEnd() { return (pointer == &headList); }
    ...
    L2ListHeader& elementAfter() { return *pointer; }

    L2ListHeader& elementBefore() { return *(pointer->prev); }
    ...
    {
        if (inBeg()) throw L2ListException("removeBefore: Beginning of list");
        else
	{
    ...
    {
        if (inEnd()) throw  L2ListException("removeAfter: End of list");
        else
	{
    ...
    {
	int s = n - pointerPos;
	if (s < 0) while (!inBeg() && s != 0) { moveBack(); ++s; }
	else if (s > 0) while (!inEnd() && s != 0) { moveForward(); --s; }
        return pointerPos;
    }
    ...
    public:
        iterator(): current(0) { }
        iterator(L2ListHeader* h): current(h) { }
        ...
        L2ListHeader& operator*() const { return *current; }
    ...
    public:
        ...
        const_iterator(const L2ListHeader* h): iterator((L2ListHeader*) h) {}
        const_iterator(const iterator& i): iterator(i) {}
        const L2ListHeader& operator*() const { return ((iterator*) this)->operator*(); }
        const L2ListHeader* operator->() const { return &(((iterator*) this)->operator*()); }
    };
