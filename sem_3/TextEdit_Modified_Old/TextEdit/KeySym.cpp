...
public:
  MyWindow()
      : // Constructor
        keycode(0),
        state(0), key(0), keyNameLen(0), wasQ(false), textFont(0), fontStruct(),
        bgColor(0), fgColor(0), dy(0), ascent(0), margin(8) {
    keyName[0] = 0;
  }
...
void MyWindow::createWindow() {
  ...
  GWindow::createWindow(I2Rectangle(         // Window frame rectangle:
...
void MyWindow::onExpose(XEvent &event) {
  ...
  if (keycode != 0) {
    ...
    sprintf(line, "XKeyEvent: keycode=0x%x, state=0x%x", keycode, state);
    drawString(x, y, line);
    y += dy;

    sprintf(line, "XLookupString: keysym=0x%x, keyNameLen=%d, keyName=\"%s\"",
...
void MyWindow::onKeyPress(XEvent &event) {
  ...
  keyNameLen = XLookupString(&(event.xkey), keyName, 255, &key, 0);
