...
const struct TextEdit::CommandDsc TextEdit::editorCommands[] = {
    ...
    {XK_KP_Down, 0, ControlMask, false,
     &TextEdit::onDown}, // down arrow on keypad

    // Up
    {XK_Up, 0, ControlMask, false, &TextEdit::onUp}, // up arrow
    {XK_KP_Up, 0, ControlMask, false,
     &TextEdit::onUp}, // up arrow on right keypad

    // Left
    {XK_Left, 0, ControlMask, false, &TextEdit::onLeft}, // left arrow
    {XK_KP_Left, 0, ControlMask, false,
     &TextEdit::onLeft}, // left arrow on keypad

    // Right
    {XK_Right, 0, ControlMask, false, &TextEdit::onRight}, // right arrow
    {XK_KP_Right, 0, ControlMask, false,
     &TextEdit::onRight}, // right arrow on keypad
    ...
    {XK_KP_Left, ControlMask, ControlMask, false,
     &TextEdit::onHome}, // Ctrl+left on keypad

    // To the end of line
    {XK_End, 0, ControlMask, false, &TextEdit::onEnd},    // End
    {XK_KP_End, 0, ControlMask, false, &TextEdit::onEnd}, // End on right keypad
    {XK_Right, ControlMask, ControlMask, false,
     &TextEdit::onEnd}, // Ctrl+right arrow
    {XK_KP_Right, ControlMask, ControlMask, false,
     &TextEdit::onEnd}, // Ctrl+right keypad

    // To the beginning of text
    {XK_Home, ControlMask, ControlMask, false,
     &TextEdit::onTextBeg}, // Ctrl+Home
    {XK_KP_Home, ControlMask, ControlMask, false,
     &TextEdit::onTextBeg}, // Ctrl+Home keypad
    {XK_Up, ControlMask, ControlMask, false, &TextEdit::onTextBeg}, // Ctrl+Up
    {XK_KP_Up, ControlMask, ControlMask, false,
     &TextEdit::onTextBeg}, // Ctrl+Up

    // To the end of text
    {XK_End, ControlMask, ControlMask, false, &TextEdit::onTextEnd}, // Ctrl+End
    {XK_KP_End, ControlMask, ControlMask, false,
     &TextEdit::onTextEnd}, // Ctrl+End
    {XK_Down, ControlMask, ControlMask, false,
     &TextEdit::onTextEnd}, // Ctrl+Down
    {XK_KP_Down, ControlMask, ControlMask, false,
     &TextEdit::onTextEnd}, // Ctrl+Down keypad
    ...
    {XK_ISO_Left_Tab, ShiftMask, ShiftMask, false,
     &TextEdit::onTabLeft}, // Shift+Tab
    ...
    // Delete line
    {XK_Delete, ShiftMask, ShiftMask, true,
     &TextEdit::onDeleteLine}, // Shift+Delete
    {XK_KP_Delete, ShiftMask, ShiftMask, true,
     &TextEdit::onDeleteLine}, // Shift+Delete on keypad
    ...
    // Insert line
    {XK_Insert, ShiftMask, ShiftMask, true,
     &TextEdit::onInsertLine}, // Shift+Insert
    {XK_KP_Insert, ShiftMask, ShiftMask, true,
     &TextEdit::onInsertLine}, // Shift+Insert on keypad
...
TextEdit::TextEdit()
    :            // Constructor
      ...
      UndoChar(0), UndoLine(0), // zero pointer!!!
      ...
      topMargin(4), statusLineMargin(4), rightMargin(4), bottomMargin(4),

      fileName("noname.txt"), // Path to a file
      fileNameSet(false),     // File name is assigned
      endOfText("[* End of text *]"),

      textChanged(false), textSaved(false), inputDisabled(false), focusIn(true),

      bgColor(0), fgColor(0), bgStatusLineColor(0), fgStatusLineColor(0) {}
...
// It fails to load good font, but it don't want to use FIXED if this fail
// occurs!!!
#ifdef __USE_FONTS__
...
void TextEdit::onExpose(XEvent &event) {
  ...
  fillRectangle(I2Rectangle(0, topMargin - statusLineMargin, m_IWinRect.width(),
                            m_IWinRect.height()));
...
void TextEdit::drawCursor(int cx, int cy, bool on,
                          bool createGC /* = false */) {
  bool cursorOn = on;
  if (!focusIn || inputDisabled)
    cursorOn = false;
  if (cx < windowX || cy < windowY || cx >= windowX + windowWidth ||
  ...
  if (cy <= text.size()) {
    ...
    if (cx < line->length())
      drawString(x, y + ascent, line->getString() + cx, 1);
  }
...
void TextEdit::postProcessCommand() {
  ...
  if (cursorX < windowX || cursorX > windowX + windowWidth - 1 ||
...
void TextEdit::UndoLineDelete() {
  ...
  // IMPORTANT: VVB wants this object (in brackets below) to be in the heap and
...
void TextEdit::UndoLineBreak() {
  if (cursorY >= text.size())
    return; // actually, this is not possible, but one more check is useful.
  text.setPointer(cursorY);
...
void TextEdit::scrollLeft(int n) {
  ...
  if (n > windowWidth / 2)
    ...
    int shift = dx * n;
    XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin, topMargin,
              windowWidth * dx - shift, windowHeight * dy, leftMargin + shift,
              topMargin);
    redrawRectangle(
        I2Rectangle(leftMargin, topMargin, shift, windowHeight * dy));
  }
  LastAction = LA_UNDEF;
}
...
void TextEdit::scrollRight(int n) {
  ...
  if (n > windowWidth / 2)
    ...
    int shift = dx * n;
    XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin + shift,
              topMargin, windowWidth * dx - shift, windowHeight * dy,
              leftMargin, topMargin);
    redrawRectangle(I2Rectangle(leftMargin + windowWidth * dx - shift,
                                topMargin, shift, windowHeight * dy));
  }
  LastAction = LA_UNDEF;
}
...
void TextEdit::scrollUp(int n) {
  ...
  if (n > windowHeight / 2)
    ...
    int shift = dy * n;
    XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin, topMargin,
              windowWidth * dx, windowHeight * dy - shift, leftMargin,
              topMargin + shift);
    redrawRectangle(
        I2Rectangle(leftMargin, topMargin, windowWidth * dx, shift));
  }
  LastAction = LA_UNDEF;
}
...
void TextEdit::scrollDown(int n) {
  ...
  if (n > windowHeight / 2)
    ...
    int shift = dy * n;
    XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin,
              topMargin + shift, windowWidth * dx, windowHeight * dy - shift,
              leftMargin, topMargin);
    redrawRectangle(I2Rectangle(leftMargin,
                                topMargin + windowHeight * dy - shift,
...
bool TextEdit::processQuit() {
  ...
  if (textChanged) {
    ...
    case SaveDialog::BUTTON_YES:
      onSave();
      break;
    case SaveDialog::BUTTON_NO:
      break; // <----  do nothing
    case SaveDialog::BUTTON_CANCEL:
      quit = false;
      break;
    }
  }
...
void TextEdit::redrawTextRectangle(int x, int y, int w, int h,
                                   bool createGC /* = false */) {
  int x1 = x + w;
  if (w == INT_MAX)
    x1 = INT_MAX;
  ...
  if (!createGC)
    redrawRectangle(I2Rectangle(left, top, width, height));
  else {
    // Erase a window
    setForeground(bgColor);
...
// B0RI$ENK0 must die... fucking classes!... fucking tExTeDiT!... fucking style
// of programming... BLYAAA!!!
///////////////////////////////////////
// class SaveDialog, Implementation
///////////////////////////////////////
SaveDialog::SaveDialog(TextEdit *e)
    : GWindow(), editor(e), yesButtonRect(), noButtonRect(), cancelButtonRect(),
      buttonWidth(0), buttonHeight(0), dialogWidth(0), dialogHeight(0),
      buttonColor1(0), buttonColor2(0),
...
void SaveDialog::create() {
  ...
  setFgColorName("black");
  m_IWinRect =
      I2Rectangle(editor->m_IWinRect.left() + 30, editor->m_IWinRect.top() + 30,
...
void SaveDialog::onExpose(XEvent &event) {
  ...
  drawString(editor->leftMargin, editor->statusLineMargin + editor->ascent,
...
void SaveDialog::drawButton(const I2Rectangle &rect, const char *text) {
  ...
  drawString(rect.left() + skip,
             rect.top() + editor->statusLineMargin + editor->ascent, text);
}
