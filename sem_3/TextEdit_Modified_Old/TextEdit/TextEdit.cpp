...
const struct TextEdit::CommandDsc TextEdit::editorCommands[] = {
    ...
    // Undo
    {XK_u, ControlMask, ControlMask, false, &TextEdit::onUndo},    	// Ctrl+u
    {XK_U, ControlMask, ControlMask, false, &TextEdit::onUndo}, 	// Ctrl+U
...
TextEdit::TextEdit():   // Constructor
    ...
    LastAction(LA_UNDEF), // INIT LAST ACTION
    UndoChar(0),
    UndoLine(0), // zero pointer!!!
...
static const char* const DEFAULT_TEXT_FONT = "-adobe-courier-medium-r-normal-*-14-*-*-*-*-*-koi8-r";

void TextEdit::loadTextFont()
{
    bool success = false;
// VVB=LAMER! This code works bad on my Mandrake 8.2!!!
// It fails to load good font, but it don't want to use FIXED if this fail occurs!!!
#ifdef __USE_FONTS__
    // At first, we read the value of XMIMFONT variable
    const char *fontString = getenv("XMIMFONT");
    if (fontString != 0) if ((textFont = loadFont(fontString)) != 0) success = true;
    // Then, try to load the default fomt
    if (!success) if ((textFont = loadFont(DEFAULT_TEXT_FONT)) != 0) success = true;
    // At last, try to load a font with the name "fixed"
    // (the "fixed" must be set as a font alias in any X-window system)
#endif
    if (!success) if ((textFont = loadFont("fixed")) != 0) success = true;
    if (!success) { perror("Cannot load a font with name \"fixed\""); exit(1); }
...
{
    redrawRectangle(I2Rectangle(0, 0, m_IWinRect.width(), dy + statusLineMargin));
}
...
void TextEdit::drawStatusLine(bool createGC /* = false */)
{
    ...
    fillRectangle(I2Rectangle(0, 0, m_IWinRect.width(), dy + statusLineMargin));
    ...
    if (textChanged) drawString(x + 19*dx, y, "Modified");
    else if (textSaved) drawString(x + 19*dx, y, "Saved");
...
void TextEdit::drawCursor(int cx, int cy, bool on, bool createGC /* = false */)
{
    ...
    if (cursorOn) setForeground(fgColor); else setForeground(bgColor);
    ...
    if (cursorOn) setForeground(bgColor); else setForeground(fgColor);
...
void TextEdit::onKeyPress(XEvent& event) {
    ...
    keyNameLen = XLookupString(&(event.xkey), keyName, 255, &keySymbol, 0);
    ...
    {
        if (keySymbol == command->keysym && (state & command->stateMask) == command->state) { commandFound = true; break; }
        ++command;
    }
    ...
    if (commandFound)
    {
        ...
        if (command->change) textChanged = true;  // Command changes the text
    }
...
{
    // Window is destroyed => return
    if (m_Window == 0) return;
    if (textChanged) textSaved = false;
...
{
    focusIn = true;
    if (!inputDisabled) drawCursor(cursorX, cursorY, true, true);
}
...
void TextEdit::onButtonPress(XEvent& event)
{
    ...
    if (cx >= windowWidth) cx = windowWidth - 1;
    cx += windowX;

    int cy = (y - topMargin) / dy;
    if (cy >= windowHeight) cy = windowHeight - 1;
    cy += windowY;
    if (cy > text.size()) cy = text.size();
...
void TextEdit::onResize(XEvent& event)
{
    ...
    if (windowWidth <= 0) { windowWidth = 1; rightMargin = 0; }

    int h = m_IWinRect.height();
    windowHeight = (h - topMargin) / dy;
    bottomMargin = h - topMargin - windowHeight * dy;
    if (windowHeight <= 0) { windowHeight = 1; bottomMargin = 0; }

    //... redraw();
}

void TextEdit::UndoLineDelete()
{
    LastAction = LA_UNDEF;
    if (!UndoLine) return;
    text.setPointer(cursorY);
    printf("UndoLineDelete(): '%s'\n", UndoLine->getString());
    // IMPORTANT: VVB wants this object (in brackets below) to be in the heap and it must live forever...!!!;-);-);-)
    TextLine * L = new TextLine(*UndoLine); 
    text.addAfter(L);
    delete UndoLine; UndoLine = 0;
    redrawTextRectangle(0, cursorY, INT_MAX, INT_MAX);
}

void TextEdit::UndoLineBreak()
{
    if (cursorY >= text.size()) return; // actually, this is not possible, but one more check is useful.
    text.setPointer(cursorY);
    TextLine& Next = text.getLine(cursorY);
    TextLine& Current = text.getLine(cursorY-1);
    Current += Next;
    Current.trim();
    onDeleteLine();
    cursorY--;
...
void TextEdit::onUndo()
{
    switch (LastAction)
    {
	case LA_UNDEF: break; // UNdo nothing
	case LA_CHARTYPED: onBackSpace(); break;
	case LA_CHARERASED: lastChar = UndoChar; onCharTyped(); break;
	case LA_LINEINSERTED: onDeleteLine(); break;
	case LA_LINEDELETED: UndoLineDelete(); break;
	case LA_LINEBREAKED: UndoLineBreak(); break;
    }
    
}

// CURSOR MOVEMENT
void TextEdit::onDown() { if (cursorY < text.size()) ++cursorY; LastAction = LA_UNDEF; }
void TextEdit::onUp() { if (cursorY > 0) --cursorY; LastAction = LA_UNDEF; }
void TextEdit::onLeft() { if (cursorX > 0)  --cursorX; LastAction = LA_UNDEF; }
void TextEdit::onRight() { ++cursorX; LastAction = LA_UNDEF; }
void TextEdit::onPageUp() { cursorY -= windowHeight; if (cursorY < 0) cursorY = 0; LastAction = LA_UNDEF; }
void TextEdit::onPageDown() { cursorY += windowHeight; if (cursorY > text.size()) cursorY = text.size(); LastAction = LA_UNDEF; }

// SCROLLINGS
void TextEdit::scrollToCursor()
{
    if (cursorX < windowX) scrollLeft(windowX - cursorX);
    else if (cursorX > windowX + windowWidth - 1) scrollRight(cursorX - (windowX + windowWidth - 1));
    if (cursorY < windowY) scrollUp(windowY - cursorY);
    else if (cursorY > windowY + windowHeight - 1) scrollDown(cursorY - (windowY + windowHeight - 1));
    LastAction = LA_UNDEF;
}

void TextEdit::scrollLeft(int n)
{
    if (n == 0) return; if (n > windowX) n = windowX; windowX -= n;
    if (n > windowWidth / 2) redraw();
    else
    {
        int shift = dx * n;
        XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin, topMargin, windowWidth * dx - shift, windowHeight * dy, leftMargin + shift, topMargin);
        redrawRectangle(I2Rectangle(leftMargin, topMargin, shift, windowHeight * dy));
    }
    LastAction = LA_UNDEF;
}
...
{
    if (n == 0) return; windowX += n;
    if (n > windowWidth / 2) redraw();
    else
    {
        int shift = dx * n;
        XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin + shift, topMargin, windowWidth * dx - shift, windowHeight * dy, leftMargin, topMargin);
        redrawRectangle(I2Rectangle(leftMargin + windowWidth*dx - shift, topMargin, shift, windowHeight * dy));
    }
    LastAction = LA_UNDEF;
}
...
{
    if (n == 0) return; if (n > windowY) n = windowY; windowY -= n;
    if (n > windowHeight / 2) redraw();
    else
    {
        int shift = dy * n;
        XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin, topMargin, windowWidth * dx, windowHeight * dy - shift, leftMargin, topMargin + shift);
        redrawRectangle(I2Rectangle(leftMargin, topMargin, windowWidth * dx, shift));
    }
    LastAction = LA_UNDEF;
}
...
{
    if (n == 0) return; windowY += n;
    if (n > windowHeight / 2) redraw();
    else
    {
        int shift = dy * n;
        XCopyArea(m_Display, m_Window, m_Window, m_GC, leftMargin, topMargin + shift, windowWidth * dx, windowHeight * dy - shift, leftMargin, topMargin);
        redrawRectangle(I2Rectangle(leftMargin, topMargin + windowHeight * dy - shift, windowWidth * dx, shift));
    }
    LastAction = LA_UNDEF;
}
// TEXT NAVIGATION
void TextEdit::onHome() { cursorX = 0; LastAction = LA_UNDEF; }
void TextEdit::onEnd() { if (cursorY >= text.size()) cursorX = 0; else cursorX = text.getLine(cursorY).length(); LastAction = LA_UNDEF; }
void TextEdit::onTextBeg() { cursorX = 0; cursorY = 0; LastAction = LA_UNDEF; }
void TextEdit::onTextEnd() { cursorX = 0; cursorY = text.size(); LastAction = LA_UNDEF; }
...
{
    ++cursorX;
    if ((cursorX % TAB_DX) != 0) cursorX += TAB_DX - (cursorX % TAB_DX);
    LastAction = LA_UNDEF; 
}

void TextEdit::onTabLeft()
{
    if (cursorX > 0) { --cursorX; if ((cursorX % TAB_DX) != 0) cursorX -= (cursorX % TAB_DX); }
    LastAction = LA_UNDEF; 
}

void TextEdit::onBackSpace()
{
    if (cursorX <= 0 || cursorY >= text.size()) return;
    LastAction = LA_CHARERASED;
    --cursorX;
    onDelete();
}
...
void TextEdit::onInsert() { lastChar = ' '; onCharTyped(); --cursorX; }

void TextEdit::onCharTyped()
{
    if (lastChar == 0) return;
    LastAction = LA_CHARTYPED;
    ...
    if (cursorX > line.length())
    {
        ...
        while(--extraSpaces >= 0) line.append(' ');   // Add space to the end of line
    }
...
{
    if (cursorY >= text.size()) return;
    LastAction = LA_LINEDELETED;
    text.setPointer(cursorY);
    UndoLine = new TextLine(text.getLine(cursorY));
    printf("onDeleteLine: UL = ' %s'\n", UndoLine->getString());
    text.removeAfter();
    redrawTextRectangle(0, cursorY, INT_MAX, INT_MAX);
}
...
void TextEdit::onEnter()
{
    ...
    if (cursorY < text.size())
    {
        ...
        text.moveForward();
        int l = line.length();
        if (cursorX >= l) text.addBefore(new TextLine());
        else
	{
            text.addBefore(new TextLine(line.getString() + cursorX));
            line.truncate(cursorX);
            line.trim();
        }
    }
    else text.addBefore(new TextLine());
    cursorX = 0;
    ++cursorY;
...
void TextEdit::onSave() {
    if (textChanged)
	if (text.save(fileName)) { textChanged = false; textSaved = true; }
}

void TextEdit::onQuit() { if (processQuit()) close(); }
...
bool TextEdit::processQuit()
{
    ...
    if (textChanged)
    {
        ...
        saveDialog.create();
        saveDialog.doModal();
        switch (saveDialog.buttonPressed)
	{
	    case SaveDialog::BUTTON_YES: onSave(); break;
	    case SaveDialog::BUTTON_NO: break;        // <----  do nothing
    	    case SaveDialog::BUTTON_CANCEL: quit = false; break;
        }
    }
    return quit;
}
...
void TextEdit::close() { destroyWindow(); }

// This virtual method is called when user presses the window close box
bool TextEdit::onWindowClosing() { return processQuit(); }

void TextEdit::redrawTextRectangle(int x, int y, int w, int h, bool createGC /* = false */)
{
    int x1 = x + w;
    if (w == INT_MAX) x1 = INT_MAX;
    int y1 = y + h;
    if (h == INT_MAX) y1 = INT_MAX;
    ...
    if (left + width > m_IWinRect.width()) width -= (left + width - m_IWinRect.width());
    if (top + height > m_IWinRect.height()) height -= (top + height - m_IWinRect.height());

    if (!createGC) redrawRectangle(I2Rectangle(left, top, width, height));
    else
    {
        // Erase a window
        setForeground(bgColor);
        fillRectangle(I2Rectangle(left, top, width, height));
	...
	{
            const TextLine* currentLine;
            if (yy > text.size()) break;
            else if (yy == text.size()) currentLine = &(endOfText);
            else currentLine = &(text.getLine(yy));
	    
            int len = currentLine->length();
            if (len > x0)
	    {
                len -= x0;
                if (len > x1 - x0) len = x1 - x0;
                drawString(left, iy, currentLine->getString() + x0, len);
            }
        }
    }
...
// B0RI$ENK0 must die... fucking classes!... fucking tExTeDiT!... fucking style of programming... BLYAAA!!!
///////////////////////////////////////
// class SaveDialog, Implementation
///////////////////////////////////////
SaveDialog::SaveDialog(TextEdit* e): GWindow(), editor(e), yesButtonRect(), noButtonRect(), cancelButtonRect(),
    buttonWidth(0), buttonHeight(0), dialogWidth(0), dialogHeight(0), buttonColor1(0), buttonColor2(0),
    buttonPressed(SaveDialog::BUTTON_CANCEL)
{
    ...
    if (w > dialogWidth) dialogWidth = w;

    int skip = (dialogWidth - 2 * editor->leftMargin - 3 * buttonWidth) / 2;
...
void SaveDialog::create() {
    ...
    setBgColorName("LightGray");
    setFgColorName("black");
    m_IWinRect = I2Rectangle(editor->m_IWinRect.left() + 30, editor->m_IWinRect.top() + 30,
...
void SaveDialog::onExpose(XEvent& event) {
    ...
    drawString(editor->leftMargin, editor->statusLineMargin + editor->ascent, "A text has been changed, save it?");
...
void SaveDialog::drawButton(const I2Rectangle& rect, const char* text)
{
    ...
    drawString(rect.left() + skip, rect.top() + editor->statusLineMargin + editor->ascent, text);
}
