...
Word::Word() : TreeSetKey(), str(0), len(0), capacity(0) {}

Word::Word(const char *s, int l) : TreeSetKey(), str(0), len(0), capacity(0) {
  assert(str != 0);
  if (l < 0)
    l = strlen(str);
  len = l;
...
Word::Word(const Word &w)
    : TreeSetKey(w), str(0), len(w.len), capacity(w.capacity) {
  if (w.len > 0) {
...
public:
  Integer() : TreeSetValue(), number(0) {}

  Integer(const Integer &i) : TreeSetValue(), number(i.number) {}

  Integer(int n) : TreeSetValue(), number(n) {}
...
int main(int argc, char *argv[]) {
  ...
  printf("The most frequent word is \"%s\", included %d times.\n",
