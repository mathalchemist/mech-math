...
public:
  ...
  Integer() : RBTreeNodeValue(), number(0) {}
  Integer(int n) : RBTreeNodeValue(), number(n) {}
  Integer(const Integer &n) : RBTreeNodeValue(), number(n.number) {}
...
void GetMaxPath(RBTreeNode *node) {
  if (!node)
    return;
  RBTreeNode *l = node->left;
  RBTreeNode *r = node->right;
...
int GetRedNodes(RBTreeNode *node) {
  if (!node)
    return 0;
  RBTreeNode *l = node->left;
  RBTreeNode *r = node->right;
  int n = GetRedNodes(l) + GetRedNodes(r);
  if (node->red)
    n++;
  return n;
}

int GetRedDepth(RBTreeNode *node) {
  if (!node)
    return 0;
  RBTreeNode *l = node->left;
  RBTreeNode *r = node->right;
  int ld = GetRedDepth(l);
  int rd = GetRedDepth(r);
  if (node->red) {
    ld++;
    rd++;
  }
...
void GetMaxRedPath(RBTreeNode *node) {
  if (!node)
    return;
  RBTreeNode *l = node->left;
  RBTreeNode *r = node->right;
...
int main() {
  ...
  while (true) {
    ...
    if (strncmp("gentree", line + commandBeg, commandLen) == 0 ||
        strncmp("gt", line + commandBeg, commandLen) == 0) {
      while (i < len && isspace(line[i]))
        ++i; // Skip a space
      ...
      while (j < n) {
        ...
        RBTreeNode *node;
        if (!tree.find(&num, tree.root(), &node)) {
    ...
    } else if (strncmp("mp", line + commandBeg, commandLen) == 0) {
      RBTreeNode *node = tree.root();
      GetMaxPath(node);
      printf("\n");
    } else if (strncmp("mrp", line + commandBeg, commandLen) == 0) {
      RBTreeNode *node = tree.root();
      printf("Red depth: %d\n", GetRedDepth(node));
      GetMaxRedPath(node);
      printf("\n");
    ...
    ///////////////////// ================================================
    else if (strncmp("quit", line + commandBeg, commandLen) == 0)
      break; // end if
  }          // end while
...
// Recursive function
static bool writeIntegerTree(const RBTreeNode *root, FILE *f,
                             int level /* = 0 */) {
  int i;
  for (i = 0; i < level; ++i) {
    fprintf(f, "    ");
...
static bool readIntegerTree(RBTree &tree, FILE *f) {
  ...
  if (i < len && (isdigit(line[i]) || line[i] == '-')) {
    ...
    if (i < len && line[i] == 'r')
      red = true;
    RBTree leftSubtree;
    if (!readIntegerTree(leftSubtree, f))
      return false;
    RBTree rightSubtree;
    if (!readIntegerTree(rightSubtree, f))
      return false;
