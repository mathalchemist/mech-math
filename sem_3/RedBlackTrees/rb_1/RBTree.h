...
public:
  ...
  RBTreeNode() : left(0), right(0), parent(0), red(false), value(0) {}
};
...
public:
  ...
  bool operator<(const RBTreeNodeValue &k) const { return (compareTo(k) < 0); }
  bool operator<=(const RBTreeNodeValue &k) const {
    return (compareTo(k) <= 0);
  }
  bool operator>(const RBTreeNodeValue &k) const { return (compareTo(k) > 0); }
...
public:
  ...
  bool find(const RBTreeNodeValue *key, const RBTreeNode *subTreeRoot = 0,
            RBTreeNodePtr *node = 0) const;
  ...
  void insert(RBTreeNode *parentNode, RBTreeNodeValue *v);
...
public:
  ...
  public:
    ...
    const_iterator(const RBTree *t, const RBTreeNode *n) : tree(t), node(n) {}
    ...
    const RBTreeNode *operator->() const { return node; }
  };
  ...
  public:
    ...
    iterator(RBTree *t, RBTreeNode *n) : const_iterator(t, n) {}

    RBTreeNode &operator*() const { // Dereference
      return (RBTreeNode &)(((const_iterator *)this)->operator*());
    }
