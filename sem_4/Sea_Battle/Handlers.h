...
{
    // network processing
    if (!WaitMessage(Socket, RecvBuf)) return;
...
void DrawCursor(int Shift) { XWin->DrawRect(Shift + 20 + CursorX * SIZE, 20 + CursorY * SIZE, Shift + 20 + (CursorX+1)*SIZE, 20 + (CursorY+1)*SIZE, RGB(255,0,0)); }

void DrawGrid(int Shift) { for (int x = 0; x < 10; x++) for (int y = 0; y < 10; y++) XWin->DrawRect(Shift + 20 + x * SIZE, 20 + y * SIZE, Shift + 20 + (x+1)*SIZE, 20 + (y+1)*SIZE, RGB(128,128,128)); }
...
void DrawField(int * Field, int Shift)
{
    ...
    for (int y = 0; y < 10; y++)
        ...
        case CS_SHIP:
        if (Shift != 0) break;
        XWin->DrawRect(Shift+20+x*SIZE,       20+y*SIZE,   Shift+20+(x+1)*SIZE,   20+(y+1)*SIZE,   RGB(255,255,255));
        XWin->DrawRect(Shift+20+x*SIZE+1,     20+y*SIZE+1, Shift+20+(x+1)*SIZE-1, 20+(y+1)*SIZE-1, RGB(255,255,255));
        break;
        case CS_HIT:
        XWin->DrawLine(Shift+20+x*SIZE,       20+y*SIZE,   Shift+20+(x+1)*SIZE,   20+(y+1)*SIZE,   RGB(255,255,255));
        XWin->DrawLine(Shift+20+(x+1)*SIZE,   20+y*SIZE,   Shift+20+x*SIZE,       20+(y+1)*SIZE,   RGB(255,255,255));
        break;
        case CS_MISS:
        XWin->DrawLine(Shift+20+x*SIZE+D,     20+y*SIZE+D, Shift+20+(x+1)*SIZE-D, 20+(y+1)*SIZE-D, RGB(255,255,255));
        XWin->DrawLine(Shift+20+(x+1)*SIZE-D, 20+y*SIZE+D, Shift+20+x*SIZE+D,   20+(y+1)*SIZE-D,   RGB(255,255,255));
        break;
        }
}
...
void ExposeHandler()
{
    ...
    {
	char*m2 = (char*)"Set your ships: WASD -- cursor move, SPACE -- set/erase";
	XWin->Write(20, 10, m2);
	DrawField((int*)MyField, 0);
    DrawCursor(0);
    char*m1 = (char*)"Hurry, your opponent is ready!";
    if (HeIsReady) XWin->Write(20, 20 + 10 * SIZE + 10, m1);
    }
    ...
    char str[256];
    if (MyTurn) sprintf(str, "YOUR TURN!     Your hits: %02d  his hits: %02d", MyHits, HisHits);
    else        sprintf(str, "PLEASE WAIT    Your hits: %02d  his hits: %02d", MyHits, HisHits);
...
int KeyHandler(int Socket)
{
    ...
    {
	if (GameState == GS_END) { Working = false; return 1;}
	SendBuf[0] = 1024;
	SendBuf[1] = 1023;
    ...
    {
    	if (KInfo.KeyCode == XK_w && CursorY > 0) CursorY--; 
    	if (KInfo.KeyCode == XK_s && CursorY < 9) CursorY++; 
    	if (KInfo.KeyCode == XK_a && CursorX > 0) CursorX--; 
    	if (KInfo.KeyCode == XK_d && CursorX < 9) CursorX++; 
    }   
 
    if (GameState==GS_INPUT && KInfo.KeyCode == XK_space)
    	if (MyField[CursorX][CursorY] == CS_EMPTY) MyField[CursorX][CursorY] = CS_SHIP; else MyField[CursorX][CursorY] = CS_EMPTY;
    
    if (KInfo.KeyCode == XK_R && GameState == GS_INPUT)
    {
    int NumFilled = 0;
    for (int x = 0; x < 10; x++)
        for (int y = 0; y < 10; y++) if (MyField[x][y] == CS_SHIP) NumFilled++;
    if (NumFilled != 20) { printf("Wrong number of ships. Check your position.\n"); return 0; }
    if (!CheckField()) { printf("Wrong position!. Check your position.\n"); return 0; }
    ...
    if (MyTurn && KInfo.KeyCode == XK_o && GameState == GS_GAME && HisField[CursorX][CursorY] != CS_HIT && HisField[CursorX][CursorY] != CS_MISS)
    {
    // send fire command
    SendBuf[0] = CursorX;
    SendBuf[1] = CursorY;
...
void SigHandler(int SigID) { printf("sighandler(): Broken pipe. SID: %d", SigID); }
