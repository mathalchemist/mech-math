...
/* Temporary variables */
static long_number d = {0, 1, NULL};
static long_number f = {0, 1, NULL};
static long_number g = {0, 1, NULL};
...
void lmadd(/* c := (a+b)(mod m) */
           const long_number *a, const long_number *b, const long_number *m,
           long_number *c) {
  lnadd(a, b, c);
  if (lnge(c, m))
    lndiv(c, m, NULL, c);
...
void lmpow(/* c := (a^n)(mod m) */
           const long_number *a, int n, const long_number *m, long_number *c) {
  int k;
...
void lmlpow(/* c := (a^n)(mod m) */
            const long_number *a, const long_number *n, const long_number *m,
            long_number *c) {
  /* Get memory */
  lminit(2 * (max(max(a->number_of_digits, m->number_of_digits),
                  n->number_of_digits) +
              2));
...
int lminv(/* c := (a)^(-1)(mod m) */
          const long_number *a, const long_number *m, long_number *c) {
