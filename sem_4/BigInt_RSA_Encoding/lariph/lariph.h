...
extern int lnndig10(/* Approximate number of decimal digits (upper bound) */
                    const long_number *n);
extern int
lntextl(/* Approximate length of text representation (upper bound) */
...
extern void lneuc(/* a*u + b*v = d = gcd(a,b) */
                  const long_number *a, const long_number *b, long_number *u,
...
inline int lneven(const long_number *a) { return ((a->digits[0] & 1) == 0); }
inline int lnodd(const long_number *a) { return ((a->digits[0] & 1) != 0); }
#endif /* __cplusplus */
...
extern int laa2nd(int len, const char *text);
...
extern int latextl(/* Approximate length of text representation (upper bound) */
                   int nd, int s,
                   const byte2 d[] /* including the terminating zero */
                   );
...
inline byte2 LA_LOWER_DIGIT(byte4 x) { return (byte2)x; }
#endif /* __cplusplus */
