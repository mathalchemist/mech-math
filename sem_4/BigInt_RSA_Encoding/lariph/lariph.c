...
void laadd(/* Addition */
           int nd1, int nd2, int *nd3, int s1, int s2, int *s3,
...
void lacopy(/* Copy numbers a2 := a1 */
            int nd1, int *nd2, int s1, int *s2, const byte2 a1[], byte2 a2[]) {
  const byte2 *p;
  byte2 *q;
  *nd2 = nd1;
  *s2 = s1;
...
int lacmp(/* Comparing: returns -1, 0, 1 */
          int nd1, int nd2, int s1, int s2, const byte2 a1[],
          const byte2 a2[]) {
  int na, nb;
  int s;
...
void lnadd(/* c := a+b */
  ...
  laadd(a->number_of_digits, b->number_of_digits, &(c->number_of_digits),
        a->sign, b->sign, &(c->sign), a->digits, b->digits, c->digits);
}

void lnsub(/* c := a-b */
           const long_number *a, const long_number *b, long_number *c) {
  lasub(a->number_of_digits, b->number_of_digits, &(c->number_of_digits),
        a->sign, b->sign, &(c->sign), a->digits, b->digits, c->digits);
}

void lnmul(/* c := a*b */
           const long_number *a, const long_number *b, long_number *c) {
  lamul(a->number_of_digits, b->number_of_digits, &(c->number_of_digits),
        a->sign, b->sign, &(c->sign), a->digits, b->digits, c->digits);
}
...
void lndiv(/* Division */
  ...
  ladiv(a->number_of_digits, b->number_of_digits, c_ndig, d_ndig, a->sign,
        b->sign, c_sign, d_sign, a->digits, b->digits, c_digits, d_digits);
}

int lncmp(/* Comparing: returns -1, 0, 1 */
          const long_number *a, const long_number *b) {
  return lacmp(a->number_of_digits, b->number_of_digits, a->sign, b->sign,
               a->digits, b->digits);
}

void lncopy(const long_number *from, long_number *to) {
  lacopy(from->number_of_digits, &(to->number_of_digits), from->sign,
         &(to->sign), from->digits, to->digits);
}
