...
TXWin::TXWin(char *Title, int X, int Y, int Width, int Height, int RGBMode) {
  ...
  if (!Disp)
    throw Ex("Cannot open display");
  Scr = DefaultScreen(Disp);
  Win = XCreateSimpleWindow(Disp, DefaultRootWindow(Disp), X, Y, Width, Height,
                            0, BlackPixel(Disp, Scr), WhitePixel(Disp, Scr));
  ...
  if (!GLContext)
    throw Ex("Cannot create context");
}
...
bool TXWin::GetEvent(int *EventType, TKeyInfo *KeyInfo, TMouseInfo *MouseInfo) {
  ...
  if (!XCheckWindowEvent(Disp, Win,
                         ExposureMask | ButtonPressMask | KeyPressMask, &Event))
    return false;
  switch (Event.type) {
  case Expose:
    *EventType = ET_EXPOSE;
  ...
  case KeyPress:
    ...
    if (XLookupString(&Event.xkey, &KeyInfo->KeyCode, 1, &KeyInfo->KeyData,
                      0) == 1)
      *EventType = ET_KEYPRESS;
    break;
  case ButtonPress:
    ...
    case Button1:
      MouseInfo->Button = MB_LEFT;
      break;
    case Button2:
      MouseInfo->Button = MB_MIDDLE;
      break;
    case Button3:
      MouseInfo->Button = MB_RIGHT;
      break;
    case Button4:
      MouseInfo->Button = MB_MWHEELUP;
      break;
    case Button5:
      MouseInfo->Button = MB_MWHEELDOWN;
      break;
    default:
      MouseInfo->Button = MB_UNKNOWN;
    }
