...
bool alive = true; // true if program is working
  ...
  while (alive & Sock != 0) {
    ...
    if (RecvData(Sock, &Buf, &address) > 0) {
      ...
      case MT_QUIT_FS:
        ...
        break;
      case MT_KILL_FS:
        cout << "Angry user " << UserName
             << " killed the server. Press ENTER to exit.\n";
        alive = false;
        break;
      }
    }
...
int SendData(int s, string *buffer, sockaddr_in *addr) {
  ...
  int res = sendto(s, buffer->c_str(), buffer->length() + 1, 0,
                   (sockaddr *)addr, len);
