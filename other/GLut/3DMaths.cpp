// =================================
// DMVN_3DMaths.cpp
// 3D Maths library implemaentation.
// (C) DMVN Corporation, 2002.
// =================================
...
TVector& TVector::operator *= (const TMatrix& M)
{
	...
	{
		double Value = 0;
		for (J = 0; J < 4; J++) Value += Coords[J] * M.Elements[I][J];
		R.Coords[I] = Value;
	}
	*this = R;
	return *this;
...
TVector operator * (const TVector& V, const TMatrix& M)
{
	...
	{
		double Value = 0;
		for (J = 0; J < 4; J++) Value += V.Coords[J] * M.Elements[I][J];
		R.Coords[I] = Value;
	}
	return R;
}
...
TMatrix& TMatrix::operator *= (const TMatrix& M)
{
	...
	for (I = 0; I < 4; I++)
		...
		{
			double Value = 0;
			for (K = 0; K < 4; K++) Value += Elements[I][K] * M.Elements[K][J];
...
TMatrix operator * (const TMatrix& A, const TMatrix& B)
{
	...
	for (I = 0; I < 4; I++)
		...
		{
			double Value = 0;
			for (K = 0; K < 4; K++) Value += A.Elements[I][K] * B.Elements[K][J];
			R.Elements[I][J] = Value;
		}
	return R;
}
